<section id="sejarahHMTGGEOI">
    <div class="container mx-auto mt-32 px-32 sm:px-6 lg:px-16 xl:px-32">
        <div class="title mb-12">
            <h1 class="font-extrabold text-4xl text-orange-primary antialiased ">Struktur Organisasi</h1>
        </div>
        <div class="visual-image w-full ">
            <a data-fancybox="gallery" href="{{ asset('/images/struktur-organisasi-1.jpg') }}">
                <img src="{{ asset('/images/struktur-organisasi-1.jpg') }}" alt="Foto Profile"
                    class="object-cover rounded-xl bg-orange-50 rounded-full shadow-md transition duration-300 ease-in-out transfrom hover:shadow-xl">
            </a>
        </div>
        <div class="visual-image w-full mt-6">
            <a data-fancybox="gallery" href="{{ asset('/images/struktur-organisasi-2.jpg') }}">
                <img src="{{ asset('/images/struktur-organisasi-2.jpg') }}" alt="Foto Profile"
                    class="object-cover rounded-xl bg-orange-50 rounded-full shadow-md transition duration-300 ease-in-out transfrom hover:shadow-xl">
            </a>
        </div>

    </div>
</section>
